...
const uploadToCloudinary = (fileBuffer) => {
    return new Promise((resolve, reject) => {
        const stream = cloudinary.uploader.upload_stream({ folder: "Restaurant_Booking_platform" }, (error, result) => {
            if (error)
                return reject(error);
            if (!result)
...
export const getOwnerBookings = async (req, res) => {
    try {
        const restaurant = await Restaurant.findOne({ owner: req.user?._id });
        if (!restaurant) {
            res.status(404).json({ message: "Restaurant profile not fonud " });
            return;
        }
...
export const updaterBookingStatus = async (req, res) => {
    try {
